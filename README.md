...
These workbooks are generated by [`scripts/generate_manifest_xlsx.py`](scripts/generate_manifest_xlsx.py) and kept in sync automatically by the [`generate-manifest-xlsx`](.github/workflows/generate-manifest-xlsx.yml) GitHub Action:

- On push to `main` touching `docs/` or `manifest_templates/`, the workflow regenerates the workbooks and commits them into `excel_templates/`.
- On a pull request touching those paths, the workflow builds the workbooks and posts a PR comment linking to them as a downloadable build artifact, so reviewers can preview the workbook before merge.
